...
from .doctor import EXPECTED_CONSOLE_SCRIPTS
...
def audit_repository(root: Path) -> dict[str, Any]:
    ...
    checks.append(AuditCheck("json-parse", not invalid_json, f"files={len(parsed_json)}"))
    ...
    for path, payload in parsed_json.items():
        ...
        for relative in REPO_REFERENCE.findall(serialized):
            if not (root / relative).is_file():
                missing_repo_refs.append(f"{path.relative_to(root).as_posix()} -> {relative}")
    checks.append(
        AuditCheck(
    ...
    blockers.extend(
        f"WORKFLOW_PERMISSIONS_MISSING:{item}" for item in workflow_permission_failures
    )
