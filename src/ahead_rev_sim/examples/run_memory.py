...
def run_rexch_round_trip() -> None:
    print("=" * 65)
    print("REXCH ROUND-TRIP (register <-> memory exchange)")
    print("=" * 65)
    print()
    ...
    print(f"  mem[16] = {m.memory.load_word(16)}")
    print()
    print("Exchange is self-inverse: no history entry was needed.")
    print()
...
def run_history_cost_analysis() -> None:
    ...
    for i in range(10):
        history.push(pc=i, op_name="REXCH", entry_type=EntryType.REVERSIBLE_OP, payload=None)
    rexch_bits = history.current_bits
    for i in range(5):
        history.push(pc=10 + i, op_name="BEQ", entry_type=EntryType.BRANCH_DECISION, payload=None)
    branch_bits = history.current_bits - rexch_bits
    print(f"10 x REXCH ops: {rexch_bits} bits")
