...
RADD  rd, rs1      ; rd = rd + rs1 mod 2^32 (inverse: subtract mod 2^32)
RSWAP rd, rs1      ; swap rd <-> rs1 (self-inverse)
RLOAD  rd, rs1, imm ; rd <-> mem[rs1+imm] (exchange, self-inverse)
RSTORE rd, rs1, imm ; rd <-> mem[rs1+imm] (exchange alias)
```
