...
def test_registry_is_packaged_and_machine_readable() -> None:
    ...
    assert len(registry["records"]) == 28
...
def test_packaged_manifest_expands_all_shards_without_loss() -> None:
    ...
    assert sum(item["record_count"] for item in manifest["shards"]) == 28
    assert len({item["shard_id"] for item in manifest["shards"]}) == len(manifest["shards"])
    assert all((manifest_path.parent / item["resource"]).exists() for item in manifest["shards"])
...
def test_registry_contains_the_frontier_and_completion_ecosystems() -> None:
    registry = load_registry()
...
def test_harvest_report_is_deterministic_and_prioritized() -> None:
    ...
    assert first["summary"]["record_count"] == 28
    assert first["summary"]["priority_counts"] == {"1": 14, "2": 11, "3": 3}
...
def test_category_selection_does_not_change_registry_authority() -> None:
    ...
    report = build_harvest_report(
        registry,
        categories=["thermodynamic_computing", "software_baseline"],
        priority_max=5,
    )
...
def test_every_source_and_asset_uses_an_explicit_https_locator() -> None:
    registry = load_registry()
    for record in registry["records"]:
        assert all(source["url"].startswith("https://") for source in record["official_sources"])
        assert all(asset["locator"].startswith("https://") for asset in record["commodity_assets"])
