...
EXECUTION_TARGET_WORKFLOW = ROOT / ".github" / "workflows" / "execution-target.yml"
...
def test_execution_target_workflow_is_directly_runnable_and_reusable() -> None:
    workflow = EXECUTION_TARGET_WORKFLOW.read_text(encoding="utf-8")
    assert "workflow_call:" in workflow
    assert "pull_request:" in workflow
    assert "push:" in workflow
...
def test_release_waits_for_hardware_model_requalification_before_publication() -> None:
    ...
    assert "execution-target:" in workflow
    assert "uses: ./.github/workflows/execution-target.yml" in workflow
    ...
    assert "ahead-rev-target --help" in workflow
    assert "gh release create" in workflow

    rtl_position = workflow.index("uses: ./.github/workflows/rtl-attachment.yml")
    target_position = workflow.index(
        "uses: ./.github/workflows/execution-target.yml"
    )
    ...
    assert target_position < release_position < publish_position
